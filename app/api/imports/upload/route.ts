...
export async function POST(request: Request) {
  const accessDenied = await sensitiveApiAccess("/api/imports/upload", ["admin", "super_admin"]);
  ...
  try {
    ...
    if (existingImport) {
      return NextResponse.json(
        {
          error:
            "An import already exists for this billing month. Unlink or delete it first if you want to replace it.",
        },
        { status: 409 }
      );
    }
    ...
    if (insertError || !importRow) {
      return NextResponse.json(
        { error: insertError?.message || "Failed to create import row" },
        { status: 500 }
      );
    }
