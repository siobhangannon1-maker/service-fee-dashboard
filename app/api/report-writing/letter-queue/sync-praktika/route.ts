...
export async function POST(req: Request) {
  try {
    ...
    if (!Array.isArray(parsedRows)) {
      return NextResponse.json(
        {
          success: false,
          error: "Praktika did not return a valid appointment array.",
          preview: responseText.slice(0, 500),
        },
        { status: 500 }
      )
  ...
  providerMap.set(
    ...
    const queueRows = (parsedRows as PraktikaAppointmentRow[])
    ...
    const matchedProviders = queueRows.filter((row) => row.provider_id).length
    const unmatchedProviders = queueRows.filter((row) => !row.provider_id).length
