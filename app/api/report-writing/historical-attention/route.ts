...
export async function POST(req: Request) {
  ...
  const review = body && !Array.isArray(body) && reviews.find(r => r.draftId === body.draftId);
  if (!review || Object.keys(body).length !== Object.keys(review).length + 1 ||
      typeof body.execute !== 'boolean' || Object.entries(review).some(([key, value]) => JSON.stringify(body[key]) !== JSON.stringify(value)))
    return Response.json({ success: false, error: 'Input must match an exact approved canary review.' }, { status: 400 });
  try {
    const { data, error } = await identity.supabase.rpc('disposition_historical_attention', {
      p_draft_id: review.draftId, p_fingerprint: review.fingerprint, p_manifest_digest: review.manifestDigest,
    ...
    if (!['eligible', 'dispositioned', 'already_dispositioned'].includes(data.code)) throw new Error();
