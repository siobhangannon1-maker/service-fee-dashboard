...
export async function POST(request: Request) {
  try {
    ...
    const partyIdRaw =
      ...
      body.party_id ??
      body.id ??
      body.referrerPartyId ??
      body.praktika_referrer_party_id ??
      null;
    ...
    const { error: auditError } = await supabaseAdmin
      ...
      .insert({
        ...
        event_summary: "Praktika referrer selected manually.",
        metadata: {
          partyId,
    ...
    if (auditError) {
      console.warn("Referrer selected, but audit failed:", auditError.message);
    }
