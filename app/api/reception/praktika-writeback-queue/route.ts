...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Could not load Praktika writeback queue", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ items: data || [] });
  } catch (error) {
    console.error("Praktika writeback queue GET failed", error);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Could not load writeback queue.",
      },
      { status: 500 }
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Could not fetch writeback items", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    ...
    console.error("Praktika writeback queue POST failed", error);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Could not process writeback queue.",
      },
      { status: 500 }
    );
  }
