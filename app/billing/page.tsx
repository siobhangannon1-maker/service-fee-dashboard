...
export default function BillingPage() {
  ...
  function addLabeledValueRow(
    ...
    row.getCell(4).value = value; // move to column D
  ...
  async function exportProviderStatements() {
    ...
    try {
      for (const provider of providers) {
        ...
        let row = 10;
worksheet.getCell(`A${row - 10}`).value = "Activity";
worksheet.getCell(`A${row - 10}`).font = { bold: true, size: 11 };
worksheet.getCell(`A${row - 10}`).fill = {
  type: "pattern",
  pattern: "solid",
  fgColor: { argb: "FFBBDEFB" },
};
worksheet.getCell(`<B1>row - 10</B1>`).alignment = { horizontal: "right" };
...
// Add full-width top AND bottom border across A–D
