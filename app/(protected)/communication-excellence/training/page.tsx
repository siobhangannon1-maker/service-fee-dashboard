...
async function submitQuiz(formData: FormData) {
  ...
  if (!assignmentId || !moduleId) {
    throw new Error("Assignment and module are required.");
  }
  ...
  const { data: linkedCompetencies } = await supabase
    ...
    .select("competency_id, communication_competencies(name)")
    .eq("module_id", moduleId);
  ...
  if (passed) {
    ...
    const firstCompetency = linkedCompetencies?.[0];

    await supabase.from("communication_microlearning").insert({
      user_id: user.id,
      competency_id: firstCompetency?.competency_id ?? null,
      title: `Review: ${module.title}`,
      description:
        "This short review was automatically assigned because the quiz was not passed. Review the module content and retry the quiz when ready.",
      status: "assigned",
      assigned_reason: `Quiz score ${score}% was below passing score ${module.passing_score}%.`,
      due_date: getDateDaysFromNow(7),
    });

    await supabase.from("audit_log").insert({
      action: "communication_microlearning_assigned",
