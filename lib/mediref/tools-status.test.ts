...
test("connection status requires recent authentication, not cookies, credentials or generic activity", () => {
  ...
  assert.equal(safeToolsStatus({ status: "connected", refreshed_at: "2026-09-08T11:59:30Z" }, null, now).session.status, "connected");
  for (const status of ["ready", "refreshing", "refresh_requested", "error", "expired", "waiting_for_credentials"]) {
    ...
    assert.equal(hasRecentMedirefAuthentication(session, now), false);
    assert.equal(safeToolsStatus(session, null, now).session.status, status === "ready" ? "sleeping" : status);
  }
});
  ...
  try {
    ...
    await page.route("**/*", async (route) => {
      ...
      if (path === "/api/report-writing/mediref-tools/status") {
        reads++;
        return route.fulfill({ json: safeToolsStatus({ status: sessionStatus, refreshed_at: new Date().toISOString() }, null) });
      }
