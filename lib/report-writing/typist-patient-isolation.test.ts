...
const names = ['beginPatientSelection', 'savePatientDraft', 'getLetterTextForSave', 'selectDraft', 'clearForm', 'startLetterFromQueue',
...
const A = { id: 'draft-a', patient_name: 'Synthetic A', patient_dob: '2000-01-01', referrer_name: 'Referrer A', referrer_address: 'Address A', edited_text: 'Letter A', report_type: 'consultation_report', status: 'edited_by_typist' };
...
function fixture() {
  ...
  Object.assign(state, { selectedDraft:null, activeQueueItemId:null, selectedProviderId:'provider', drafts:[A,B], queue:[],
    ...
    reportType:'consultation_report', reportTypes:[], preferredExampleId:'', queueStatusTab:'active', imageDraftId:null });
  const refs = { queueSelectionTokenRef:{current:0}, pendingPatientSavesRef:{current:{} as Values}, localDraftEditsRef:{current:new Map()}, patientSaveChainsRef:{current:new Map()},
  ...
  Object.assign(globals, refs, {
    ...
    fetch:(url:string,init?:RequestInit)=>{
      const d=deferred<unknown>(); requests.push({url,body:init?.body?JSON.parse(String(init.body)):{},reply:data=>d.resolve({ok:true,json:async()=>data,text:async()=>JSON.stringify(data)})}); return d.promise;
    },
...
test('A edit -> immediate B -> return A -> rapid switches retain isolated values and captured save',async()=>{
 const f=fixture();f.render().selectDraft(A);f.edit('Edited A');
 const cleanup=f.render().effects[0]();f.render().selectDraft(B);cleanup();
 ...
 for(const draft of [A,B,A,B]) {f.render().selectDraft(draft);assert.equal(f.state.letterText,draft.id===A.id?'Edited A':'');}
 ...
 f.render().selectDraft(A);assert.equal(f.state.letterText,'Edited A');
});
...
for(const method of ['persistCurrentPatientDetails','persistCurrentReferrerDetails']) test(method+' late completion cannot populate B',async()=>{
 const f=fixture();f.render().selectDraft(A);f.state.patientDob='2001-01-01';f.state.referrerName='Changed A';
 const save=f.render()[method]({quiet:true});await settle();f.render().selectDraft(B);
...
for(const effect of [1,2]) test('pending metadata autosave survives switching: '+effect,async()=>{
 const f=fixture();f.render().selectDraft(A);f.state.patientDob='2001-01-01';f.state.referrerName='Changed A';
 const cleanup=f.render().effects[effect]();f.render().selectDraft(B);cleanup();await settle();
...
test('late referrer response cannot populate B or cache against B',async()=>{
 const f=fixture();f.render().selectDraft(A);const lookup=f.render().autoFillReferrerFromLatestPraktikaReferral('synthetic-a','queue-a');
 f.render().selectDraft(B);const before={...f.state};f.requests[0].reply({success:true,referral:{referrerName:'Old A',referrerAddress:'Old address'}});await lookup;
...
test('current referrer response with blank address clears previous address',async()=>{
 const f=fixture();f.render().selectDraft(A);const lookup=f.render().autoFillReferrerFromLatestPraktikaReferral('synthetic-a');
...
test('linked queue restores its exact draft and standalone selection clears queue identity',async()=>{
 const f=fixture();f.render().selectDraft(A);await f.render().startLetterFromQueue({id:'queue-b',report_draft_id:B.id,patient_first_name:'Stale queue name'});
 ...
 f.render().selectDraft(A);assert.equal(f.state.activeQueueItemId,null);
});
test('late linked-draft load cannot replace a newer selection',async()=>{
 const f=fixture();f.state.drafts=[];const load=f.render().startLetterFromQueue({id:'queue-a',report_draft_id:A.id});
 f.render().selectDraft(B);f.requests[0].reply({success:true,drafts:[A]});await load;assert.equal(f.state.selectedDraft.id,B.id);assert.equal(f.state.letterText,'');
});
test('late generation response cannot replace B letter or save state',async()=>{
 const f=fixture();f.render().selectDraft(A);const generation=f.render().generateLetter();f.render().selectDraft(B);const before={...f.state};
...
test('late manual draft save still completes but cannot reselect A',async()=>{
 ...
 const save=f.render().saveNewDraft('edited_by_typist');f.render().selectDraft(B);const before={...f.state};
...
test('late image-workspace creation cannot change B workspace',async()=>{
 ...
 const work=f.render().ensureImageDraftForCurrentWork({quiet:true});f.render().selectDraft(B);
...
test('late match results cannot select or persist a match for B',async()=>{
 const f=fixture();f.render().selectDraft(A);const match=f.render().searchPraktikaPatientMatch();f.render().selectDraft(B);const before={...f.state};
...
test('old A save completion cannot replace a newer A visit',async()=>{
 const f=fixture();f.render().selectDraft(A);f.edit('First edit A');f.render().effects[0]();
 f.render().selectDraft(B);await settle();f.render().selectDraft(A);f.edit('Second edit A');f.render().effects[0]();
 ...
 f.render().selectDraft(B);await settle();assert.equal(f.requests[1].body.draftId,A.id);assert.equal(f.requests[1].body.editedText,'Second edit A');
 f.requests[1].reply({success:true});await settle();f.render().selectDraft(A);assert.equal(f.state.letterText,'Second edit A');
});
for(const method of ['persistCurrentPatientDetails','persistCurrentReferrerDetails'])test(method+' preserves explicit blanks in saved/UI metadata',async()=>{
 const f=fixture();f.render().selectDraft(A);
