...
export function remainsInApproved(draft: { status: string; emailed_to_referrer_at?: string | null;
  workflow_status?: string | null; workflow_praktika_upload_status?: string | null; workflow_icon_update_status?: string | null; workflow_mediref_status?: string | null }) {
  if (!['approved', 'uploaded_to_praktika'].includes(draft.status)) return false;
  if (draft.workflow_status) return draft.workflow_praktika_upload_status !== 'completed'
