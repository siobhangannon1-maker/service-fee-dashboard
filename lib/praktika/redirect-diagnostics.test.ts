...
test("307 diagnostics preserve background proof and strict job failure behavior, without following or reading body", async () => {
  ...
  let location = "/v2/login?PRIVATE#PRIVATE";
  ...
  try {
    ...
    assert.equal(failureWrites, 1); assert.equal(calls, 3); assert.equal(disposed, 3);
  ...
  assert.equal(fields.httpStatus, 307); assert.equal(fields.redirect_category, "login_redirect");
