...
async function getPatientPerioExamIds(patientId: string | number) {
  const response = await praktikaHelperPostForCurrentUser<any>({
    jobType: "periodontal_chart_patient_perio_exam_ids",
    priority: 25,
...
async function getPerioExams(perioExamIds: number[]) {
  if (perioExamIds.length === 0) return [];

  const response = await praktikaHelperPostForCurrentUser<any>({
    jobType: "periodontal_chart_perio_exams",
    priority: 25,
    path: "/php/forms/db_getFormData.php",
    contentType: "json",
    referer: "https://praktika.praktika.net.au/v2/scheduler",
