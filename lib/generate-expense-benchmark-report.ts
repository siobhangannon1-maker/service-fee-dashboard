...
function ensureAllowedStatus(
  ...
  throw new Error(
    `Invalid benchmark status "${String(status)}" for category "${categoryName}". Status must be green, orange, or red.`
  )
}
...
function normalizeExpenseItems(rawItems: unknown[]): NormalizedExpenseItem[] {
  ...
  for (const rawItem of rawItems) {
    ...
    const expenseAmount = Number(item.expense_amount ?? item.amount ?? item.value ?? 0)
...
async function getBillingPeriod(year: number, month: number): Promise<BillingPeriodRow> {
...
async function getGrossProductionForBillingPeriod(
  ...
  if (error) {
    throw new Error(`Failed to load gross production for ${monthKey}: ${error.message}`)
  }
  ...
  if (grossProduction <= 0) {
    throw new Error(`Gross production for ${monthKey} must be greater than 0`)
  }
...
async function saveExpenseBenchmarkReport(report: ExpenseBenchmarkReport): Promise<void> {
  const monthLabel = new Date(report.year, report.month - 1, 1).toLocaleString('en-AU', {
    month: 'long',
    year: 'numeric',
  })
...
async function generateExpenseBenchmarkReport(
  ...
  for (const item of items) {
    const mappedCategoryName =
      mappingMap.get(normalizeText(item.source_account_name)) || 'Other Expenses'
