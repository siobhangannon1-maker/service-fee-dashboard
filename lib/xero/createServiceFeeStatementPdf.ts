...
export async function createServiceFeeStatementPdf(
  ...
  async function drawHeader() {
    ...
    if (input.logoDataUrl) {
      try {
        const isPng = input.logoDataUrl.startsWith("data:image/png");
        const isJpeg =
          input.logoDataUrl.startsWith("data:image/jpeg") ||
          input.logoDataUrl.startsWith("data:image/jpg");
        ...
        console.error("Logo embed failed:", error);
      }
    }

    if (!drewLogo) {
      drawText(PRACTICE.name, margin, y - 20, {
        size: 14,
        bold: true,
      });
    }
    ...
    drawText(input.periodLabel, 400, y - 28, {
      size: 11,
      bold: true,
    });
    ...
    drawText(input.providerName, margin, y, {
      size: 13,
      bold: true,
    });
    y -= 15;
    ...
    if (input.providerAbn) {
      drawText(`ABN: ${input.providerAbn}`, margin, y, {
        size: 9,
      });
      y -= 13;
    }

    drawText(PRACTICE.name, margin, y, {
      size: 9,
      bold: true,
    });
    y -= 12;

    drawText(PRACTICE.address, margin, y, {
      size: 9,
    });
    y -= 12;

    drawText(`Ph: ${PRACTICE.phone}`, margin, y, {
      size: 9,
    });
    y -= 12;

    drawText(`Email: ${PRACTICE.email}`, margin, y, {
      size: 9,
    });
    y -= 12;

    drawText(`ABN ${PRACTICE.abn}`, margin, y, {
      size: 9,
    });
    y -= 25;
  }
  ...
  row(
    "Plus GST",
    input.labMaterialsGst + input.hummFeesGst + input.afterpayFeesGst
  );
  ...
  row(
    `Less patient fees owed to ${input.providerName}`,
    input.patientFeesPaidToFocusNoGst
  );
  row(
    `Facility fees paid to ${input.providerName}, owed to Focus`,
    input.patientFeesPaidToAnotherProviderNoGst
  );
  row(
    `Patient fee paid to ${input.providerName} in error`,
  ...
  detailSection(
    "Implant/Grafting Materials/Guides NET excluding GST",
    input.implantEntries,
    input.labMaterialsExGst
  );

  detailSection(
    "Humm Merchant Fees NET excluding GST",
    input.hummEntries,
    input.hummFeesExGst
  );

  detailSection(
    "Afterpay Fees NET excluding GST",
    input.afterpayEntries,
    input.afterpayFeesExGst
  );

  detailSection(
    "Patient fees owed to provider",
    input.paidToFocusEntries,
    input.patientFeesPaidToFocusNoGst
  );

  detailSection(
    `Patient fee paid to ${input.providerName} in error`,
    input.paidToThisProviderInErrorEntries,
    input.patientFeesReceivedInErrorNoGst
  );

  detailSection(
    "Facility fees paid to another provider, owed to Focus",
    input.paidToAnotherProviderEntries,
    input.patientFeesPaidToAnotherProviderNoGst
  );
