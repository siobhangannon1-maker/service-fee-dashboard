...
function rowHasAnyUsefulData(row: RawRow): boolean {
  return Object.values(row).some((value) => String(value ?? "").trim() !== "");
}

function rowHasRequiredCancellationData(row: RawRow): boolean {
  ...
  return Boolean(providerNameRaw && dateRaw);
}
...
export async function importProviderCancellationsFtasCsv(
  ...
  for (const row of rows) {
    if (!rowHasAnyUsefulData(row)) continue;
    if (!rowHasRequiredCancellationData(row)) continue;
