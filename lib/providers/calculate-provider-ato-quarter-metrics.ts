...
type CalculateProviderAtoQuarterMetricsParams = {
  quarterKey: string;
};

type CalculateProviderAtoQuarterMetricsResult = {
  quarterKey: string;
...
function getQuarterRangeFromQuarterKey(quarterKey: string): { start: string; end: string } {
  const match = quarterKey.match(/^(\d{4})-Q([1-4])-ATO$/);

  if (!match) {
    throw new Error(`Invalid quarter key: "${quarterKey}". Expected YYYY-QN-ATO`);
  }
  ...
  if (quarterNumber === 1) {
    return {
      ...
      end: `${financialYearStart}-09-30`,
    };
  }

  if (quarterNumber === 2) {
    return {
      start: `${financialYearStart}-10-01`,
      end: `${financialYearStart}-12-31`,
    };
  }

  if (quarterNumber === 3) {
    return {
      start: `${financialYearStart + 1}-01-01`,
      end: `${financialYearStart + 1}-03-31`,
    };
  }

  return {
    start: `${financialYearStart + 1}-04-01`,
    end: `${financialYearStart + 1}-06-30`,
  };
}
...
export async function calculateProviderAtoQuarterMetrics(
  params: CalculateProviderAtoQuarterMetricsParams
): Promise<CalculateProviderAtoQuarterMetricsResult> {
  const supabase = getServiceRoleSupabaseClient();
  const quarterRange = getQuarterRangeFromQuarterKey(params.quarterKey);
  ...
  const { data, error } = await supabase
    ...
    .gte("period_start", quarterRange.start)
    .lte("period_end", quarterRange.end);
  ...
  for (const chunk of chunks) {
    ...
    if (upsertError) {
      throw new Error(`Failed to upsert ATO quarter provider metrics: ${upsertError.message}`);
    }
  }
