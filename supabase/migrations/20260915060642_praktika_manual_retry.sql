...
begin;
create function public.reserve_praktika_upload_retry(p_draft_id uuid, p_prior_job_id uuid, p_actor_user_id uuid)
...
begin
  ...
  if not found or parent.job_type <> 'complete_report_workflow'
    or parent.app_user_id is distinct from p_actor_user_id
    or parent.request->>'actorUserId' is distinct from p_actor_user_id::text
  ...
  if not found or prior.job_type <> 'upload_report_to_praktika' or prior.status <> 'failed'
    ...
    or prior.app_user_id is distinct from p_actor_user_id
  ...
  if found then
    if replacement.app_user_id is distinct from p_actor_user_id
    ...
    with recursive lineage as (
      ...
      select j.id,j.request from public.praktika_helper_jobs j join lineage l
        ...
        where j.job_type='upload_report_to_praktika' and j.request->>'reportDraftId'=p_draft_id::text
          and j.app_user_id=p_actor_user_id and j.status='failed'
    )
  ...
  update public.praktika_helper_jobs set status='waiting', locked_at=null,locked_by=null,
    ...
    response=jsonb_build_object('stage','upload','retryUploadId',replacement_id)
    where id=parent_id;
