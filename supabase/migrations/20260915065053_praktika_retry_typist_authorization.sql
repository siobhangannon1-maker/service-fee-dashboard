...
begin
  ...
  if found then
    if replacement.app_user_id is distinct from parent.app_user_id
    ...
    with recursive lineage as (
      ...
      select j.id,j.request from public.praktika_helper_jobs j join lineage l
        ...
        where j.job_type='upload_report_to_praktika' and j.request->>'reportDraftId'=p_draft_id::text
          and j.app_user_id=parent.app_user_id and j.status='failed'
    )
  ...
  -- Keep execution/session routing with the original workflow; audit records the retry authorizer.
  insert into public.praktika_helper_jobs(id,app_user_id,job_type,status,priority,request,available_at)
    values(replacement_id,parent.app_user_id,'upload_report_to_praktika','waiting',20,
