...
export async function pollPraktikaWorkflowContinuations(
  db: SupabaseClient,
  actorId: string | null | undefined,
  ownership: Pick<PraktikaJobOwnership, "assertOwned" | "isShuttingDown">,
) {
  try {
    ...
    const { data, error } = await db
      ...
      .or(
        `and(app_user_id.eq.${actorId},response->>retryUploadId.is.null,${runnable}),` +
        `and(response->>retryUploadId.not.is.null,response->>retryExecutionUserId.eq.${actorId},${runnable})`,
      )
      // Fairness: a continuation that was just attempted must not permanently
      // outrank untouched work merely because it was originally created first.
      .order('updated_at')
      .limit(1)
    ...
    const intent = data[0];
    const executionActorId = intent.response?.retryUploadId
      ? intent.response?.retryExecutionUserId
      : intent.app_user_id;
    ...
    const { data: claimed, error: claimError } = await db
      ...
      .update({
        ...
        locked_by: lock,
        response: { ...intent.response, dispatched: false },
        updated_at: new Date().toISOString(),
      })
      .eq('id', intent.id)
      .eq('status', intent.status)
      .eq('updated_at', intent.updated_at)
      .select('id')
      .maybeSingle();
      ...
      await db
        ...
        .update({
          ...(rejectedBeforeExecution
            ? { status: 'waiting', locked_by: null, locked_at: null }
            : {}),
          response: {
            ...intent.response,
            dispatched: !rejectedBeforeExecution,
            issue,
          },
          updated_at: new Date().toISOString(),
        })
    ...
    if (configurationIssue) {
      await pause(configurationIssue);
      return;
    }
    ...
    void fetch(`${origin}/api/report-writing/continue-praktika-workflow`, {
      ...
      headers: {
        ...
        'X-Workflow-Token': continuationToken(
          intent.id,
          lock,
          process.env.SUPABASE_SERVICE_ROLE_KEY!,
        ),
      },
      body: JSON.stringify({ intentId: intent.id, lock }),
      redirect: 'error',
      ...
      .catch(async () => {
        await pause('continuation_unavailable').catch(() => {});
      })
      .finally(() => inFlight.delete(actorId));
  } catch {
    // Continuation transport failure must not stop normal helper draining.
  }
}
